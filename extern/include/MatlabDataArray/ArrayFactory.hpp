...
class ArrayFactory {
  public:
    ...
    /**
     * Creates an MATLAB Data Array-managed buffer which can be passed into createArrayFromBuffer()
    ...
    buffer_ptr_t<T> createBuffer(size_t numberOfElements) {
        ...
        return buffer_ptr_t<T>(static_cast<T*>(buffer), reinterpret_cast<buffer_deleter_t<T>>(deleter));
    }

    /**
     * Creates a TypedArray<T> using the given buffer
     * The buffer can either be
     * - MATLAB Data Array-managed, allocated by createBuffer, or
     * - user-managed, allocated by the user with a custom deleter of type buffer_deleter_t
    ...
    TypedArray<T> createArrayFromBuffer(ArrayDimensions dims,
        ...
        buffer_deleter_t<> deleter = reinterpret_cast<buffer_deleter_t<>>(buffer.get_deleter());
        impl::ArrayImpl* impl = nullptr;
    ...
    typename std::enable_if<std::is_same<double, T>::value ||
                            std::is_same<bool, T>::value || std::is_same<std::complex<double>, T>::value,
                            SparseArray<T>>::type
    createSparseArray(ArrayDimensions dims,
        ...
        buffer_deleter_t<> data_deleter = reinterpret_cast<buffer_deleter_t<>>(data.get_deleter());
        buffer_deleter_t<> rows_deleter = reinterpret_cast<buffer_deleter_t<>>(rows.get_deleter());
        buffer_deleter_t<> cols_deleter = reinterpret_cast<buffer_deleter_t<>>(cols.get_deleter());
    ...
    createSparseArray(ArrayDimensions dims,
        ...
        static const CreateSparseArrayFromBufferFcnPtr fcn =
            detail::resolveFunction<CreateSparseArrayFromBufferFcnPtr>(
                detail::FunctionType::CREATE_SPARSE_ARRAY_FROM_BUFFER_V2);
        detail::throwIfError(fcn(pImpl.get(), static_cast<int>(GetSparseArrayType<T>::type),
                                 &dims[0], dims.size(), nnz, data.release(), data_deleter,
                                 rows.release(), rows_deleter, cols.release(), cols_deleter,
                                 &impl));
